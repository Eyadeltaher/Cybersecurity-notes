...
\midrule
\rowcolor{row2}
\textcolor{commandbg}{knockpy -d "zu.edu.eg" --recon --bruteforce --json} & 
Discovers subdomains, ip and certs and saves to json file \\
\rowcolor{row1}
\lstinline|cat file.json || \lstinline|grep -oP '"domain":\s*"\K[^"]+'|
 > subk.txt & Create a txt file with discovered subdomains\\
\rowcolor{row2}
\textcolor{commandbg}{subdominator -d target.com -o subd.txt} & 
...
\rowcolor{row1}
\textcolor{commandbg}{cat subf.txt ast.txt subk.txt subd.txt | sort -u | tee subdomains.txt} & 
